      ...
      <div className="mt-14 divide-y divide-black/10 border-t border-black/10">
        {practiceAreas.map((area) => (
          <article key={area.slug} className="grid gap-3 py-10 md:grid-cols-[0.7fr_1.3fr] md:gap-10">
            <h2 className="font-[family-name:var(--font-display)] text-xl text-[var(--color-navy)]">{area.name}</h2>
            <p className="text-[15px] leading-relaxed text-[var(--color-slate)]">{area.detail}</p>
          </article>
        ))}
      </div>
    </section>
